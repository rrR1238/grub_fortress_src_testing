...
//		bool bHasVertexColor = HasFlag(MATERIAL_VAR_VERTEXCOLOR) || HasFlag(MATERIAL_VAR_VERTEXALPHA);
		...
		int nTexCoords = 2; // 2 texcoords for brushes, 1 for models
		pShaderShadow->VertexShaderVertexFormat(flags, nTexCoords, 0, 0);
		...
		// ProjTex	= lux_model simplified VS
		...
		else
		{
			...
			SET_STATIC_VERTEX_SHADER_COMBO(BUMPED_LIGHTMAP, 0);
