...
IMPLEMENT_NETWORKCLASS_ALIASED(TFPistol_EngineerSecondary, DT_WeaponPistol_EngineerSecondary)

BEGIN_NETWORK_TABLE(CTFPistol_EngineerSecondary, DT_WeaponPistol_EngineerSecondary)
END_NETWORK_TABLE()

BEGIN_PREDICTION_DATA(CTFPistol_EngineerSecondary)
END_PREDICTION_DATA()

LINK_ENTITY_TO_CLASS(tf_weapon_handgun_engineer_secondary, CTFPistol_EngineerSecondary);
PRECACHE_WEAPON_REGISTER(tf_weapon_handgun_engineer_secondary);


//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
CTFPistol_EngineerSecondary::CTFPistol_EngineerSecondary()
{
	m_flPushTime = -1.f;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFPistol_EngineerSecondary::PlayWeaponShootSound(void)
{
	BaseClass::PlayWeaponShootSound();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFPistol_EngineerSecondary::SecondaryAttack(void)
{
	extern ConVar cf_enable_shortstop_shove;
	if (!cf_enable_shortstop_shove.GetBool())
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFPistol_EngineerSecondary::Push(void)
{
	CTFPlayer* pOwner = ToTFPlayer(GetPlayerOwner());
	if (!pOwner)
		return;
...
// Input  :  - 
//-----------------------------------------------------------------------------
void CTFPistol_EngineerSecondary::ItemPostFrame()
{
	// Check for smack.
	if (m_flPushTime > -1.f && gpGlobals->curtime > m_flPushTime)
	{
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFPistol_EngineerSecondary::Holster(CBaseCombatWeapon* pSwitchingTo)
{
	m_flPushTime = -1.f;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFPistol_EngineerSecondary::Precache(void)
{
	PrecacheScriptSound("Weapon_Hands.Push");
	PrecacheScriptSound("Weapon_Hands.PushImpact");
