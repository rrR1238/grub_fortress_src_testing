...
ConVar cf_snd_penetrationkill("cf_snd_penetrationkill", "1", FCVAR_NOTIFY, "OHHHHHHH PENETRATED! - Disables the Machina penetration sound");
...
void CTFHudDeathNotice::OnGameEvent( IGameEvent *event, int iDeathNoticeMsg )
{
	...
	if ( FStrEq( pszEventName, "player_death" ) || FStrEq( pszEventName, "object_destroyed" ) || FStrEq(pszEventName, "npc_death"))
	{
		...
		// Unless you really wanna be annoying :)
		...
		if ( bPenetrateSound )
		{
			if ( cf_snd_penetrationkill.GetBool() )
			{
				CLocalPlayerFilter filter;
