...
void CItem_AmmoCrate::HandleAnimEvent( animevent_t *pEvent )
{
	if ( pEvent->event == AE_AMMOCRATE_PICKUP_AMMO )
	{
		if ( m_hActivator )
		{
			CTFPlayer* pPlayer = ToTFPlayer(m_hActivator);
			pPlayer->GiveAmmo(9999, TF_AMMO_PRIMARY, true);
			pPlayer->GiveAmmo(9999, TF_AMMO_SECONDARY, true);
			pPlayer->GiveAmmo(9999, TF_AMMO_GRENADES1, true);
			pPlayer->GiveAmmo(9999, TF_AMMO_GRENADES2, true);
			pPlayer->GiveAmmo(9999, TF_AMMO_GRENADES3, true);
			pPlayer->GiveAmmo(9999, TF_AMMO_METAL, true);
