...
public:
	...
	virtual void	Killed( const CTakeDamageInfo &info );
	virtual void	SetModel( const char *pModel );
//	virtual int	GetSentryType() const { return m_iSentryType; };
	virtual int	GetSentryType() const { return 0; };
...
private:
	...
	float m_flScaledSentry;
	bool m_bIsArtillerySentry_Rocket;
};
...
class CTFProjectile_SentryRocket_Artillery : public CTFProjectile_Rocket
{
public:
	DECLARE_CLASS(CTFProjectile_SentryRocket_Artillery, CTFProjectile_Rocket);
	DECLARE_NETWORKCLASS();

	CTFProjectile_SentryRocket_Artillery();
	bool m_bIsArtillerySentry_Rocket;
	virtual int GetProjectileType() const OVERRIDE { return TF_PROJECTILE_SENTRY_ROCKET; }

	// Creation.
	static CTFProjectile_SentryRocket_Artillery* Create(const Vector& vecOrigin, const QAngle& vecAngles, CBaseEntity* pOwner = NULL, CBaseEntity* pScorer = NULL);
